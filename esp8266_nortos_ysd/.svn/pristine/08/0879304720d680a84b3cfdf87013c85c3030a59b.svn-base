...
{
    int i = 0;
    struct esp_platform_wait_timer_param timer_wait_param[100] = {{{0},{0}}};
    ...
    for (i = 0 ; i < count ; i++) {
        ...
        if (indexof(str, "f", 0) == 0) {
            ...
            timer_wait_param[i].wait_time_second = atoi((char *)timer_wait_param[i].wait_time_param) - timestamp;
            ...
            timer_wait_param[i].wait_time_second = atoi((char *)timer_wait_param[i].wait_time_param) - (timestamp % atoi((char *)timer_wait_param[i].wait_time_param));
            os_free(loop_wait[0]);
            os_free(loop_wait[1]);
            ...
            if (atoi((char *)timer_wait_param[i].wait_time_param) > monday_wait_time) {
                timer_wait_param[i].wait_time_second = atoi((char *)timer_wait_param[i].wait_time_param) - monday_wait_time;
            } else {
                timer_wait_param[i].wait_time_second = 7 * 24 * 3600 - monday_wait_time + atoi((char *)timer_wait_param[i].wait_time_param);
            }
...
esp_platform_timer_action(struct esp_platform_wait_timer_param *timer_wait_param, uint16 count)
{
    uint16 i = 0;
    uint16 action_number;
    struct wait_param pwait_action = {{{0},{0}}};
    ...
    for (i = 0; i < count ; i++) {
        if (timer_wait_param[i].wait_time_second == min_wait_second) {
            os_memcpy((void *)pwait_action.action[action_number], (void *)timer_wait_param[i].wait_action, os_strlen((char *)timer_wait_param[i].wait_action));
