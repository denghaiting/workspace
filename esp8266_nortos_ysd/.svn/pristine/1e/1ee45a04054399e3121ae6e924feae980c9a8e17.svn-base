...
#define WAITTIME_UART_SEND (5000)   //5毫秒超时
#define CHECK_TIME_UART_SEND (200)  //波特率115200， 200微秒大概11字节。不带数据的应答字节为8字节
#define	MAX_UART_INBD_STOU_TIME	(3) //发送给下位机的重试次数
#define	CONFIG_CHECKSUM	(1)

#define	Sync1 0xFF														/* 设置同步字1					*/
#define	Sync2 0xAA													   	/* 设置同步字2					*/
#define	Sync3 0x55													   	/* 设置同步字3					*/
...
// TODO:
/* 6621所有产品CMD使用的是同一识别空间，暂时不使用 */
#define UART_CMD_PT_IS_ACTIVE      0XD6        /*串口检查是否激活*/
#define UART_CMD_PT_RESET      0XD7        /*串口产测恢复出厂设置*/
#define UART_CMD_PT_WIFI_SSID  0XD8           /*串口产测知道无线ssid连接*/
#define UART_CMD_PT_WIFI_STATUS_QUERY  0XD9 /*串口产测无线连接状态查询*/
#define UART_CMD_PT_LICENCE_ERASE  0XDA   /*串口产测证书擦除*/
#define UART_CMD_PT_GPIO_SET  0XDB        /*串口产测gpio设置*/
#define UART_CMD_PT_GPIO_QUERY 0XDC       /*串口产测gpio查询*/
#define UART_CMD_PT_GPIO_SET_V2  0XDD         /*串口产测gpio设置,V2版本，调整成通用*/
#define UART_CMD_PT_GPIO_QUERY_V2 0XDE       /*串口产测gpio查询,V2版本，调整成通用*/
#define UART_CMD_PT_GPIO_TSET     0XDF       //测试glx64s2的GPIO PC下发8266
#define factory_test            0x07         //测试glx64s2的GPIO 8266下发glx64s2
...
#define UART_RS_OK				0		/* 正常返回 */
#define UART_RS_ERROR 			1		/* 失败 */
#define UART_RS_INVALID_CHECKSUM 2		/* 接收校验失败 */
#define UAET_RS_PARAM_TOOLONG	3		/* 参数过长超过缓存区大小 */
...
typedef struct {
	...
	u_int16_t index_read;	// 参数长度 param index
...
// TODO:
#if 0
/* GPIO产测要考虑GPIO_NUM_8266和此处针对8266修改 */
