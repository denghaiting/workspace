...
#include "mem.h"
#include "os_type.h"
#include "../../iw_inc/iw_api.h"
#include "user_interface.h"
#include "../../iw_inc/iw_event.h"

// UartDev is defined and initialized in rom code.
extern UartDevice    UartDev;
uint8_t is_active_mode = 0;
...
extern void uart_div_modify(uint8 uart_no, uint32 base);
...
void 
uart0_sendStr_active(const char *str)
{
    while(*str){
        uart_tx_one_char(UART0, *str++);
    }
...
uart_recvTask(os_event_t *events)
{
    if(events->sig == 0){
        ...
        uint8 d_tmp = 0;
        uint8 idx=0;
		//os_printf("uart_recvTask fifo_len[%d] ... \n", fifo_len);
        for(idx=0;idx<fifo_len;idx++) {
            d_tmp = READ_PERI_REG(UART_FIFO(UART0)) & 0xFF;
...
uart_init(UartBautRate uart0_br, UartBautRate uart1_br)
{
    ...
    ETS_UART_INTR_ATTACH(uart0_rx_intr_handler, 0);
    ETS_UART_INTR_ENABLE();
#endif

    UART_SetPrintPort(UART_PRI_PORT);
...
void
uart0_write_char_active(char c)
{
    if (c == '\n') {
        uart_tx_one_char(UART0, '\r');
        uart_tx_one_char(UART0, '\n');
...
void  BSP_EnterActiveMode(VOID)
{
	is_active_mode = 1;
}

void  BSP_ExitActiveMode(VOID)
{
	is_active_mode = 0;	
}

uint8  BSP_ISActiveMode(void)
{
	return is_active_mode;	
}

void  BSP_ActiveUartPutcPolled(uint8 Data)
{
	uart0_write_char_active((char) Data);
}

void  BSP_ActiveUartPutstr(const char *str)
{
	uart0_sendStr_active(str);
}
