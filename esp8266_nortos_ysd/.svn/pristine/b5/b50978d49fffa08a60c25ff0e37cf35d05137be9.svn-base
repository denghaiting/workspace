...
user_devicefind_recv(void *arg, char *pusrdata, unsigned short length)
{
    ...
    char hwaddr[6];
	struct ip_addr *ip_temp;
    struct ip_info ipconfig;

	ip_temp = (struct ip_addr *)(ptrespconn.proto.udp->remote_ip);
	
    if (wifi_get_opmode() != STATION_MODE) {
        wifi_get_ip_info(SOFTAP_IF, &ipconfig);
        wifi_get_macaddr(SOFTAP_IF, (uint8 *)hwaddr);

        if (!ip_addr_netcmp(ip_temp, &ipconfig.ip, &ipconfig.netmask)) {
            wifi_get_ip_info(STATION_IF, &ipconfig);
            wifi_get_macaddr(STATION_IF, (uint8 *)hwaddr);
        }
    } else {
        wifi_get_ip_info(STATION_IF, &ipconfig);
        wifi_get_macaddr(STATION_IF, (uint8 *)hwaddr);
    ...
    if (length == os_strlen(device_find_request) &&
        ...
        espconn_sent(&ptrespconn, (uint8 *)DeviceBuffer, (uint16)length);
