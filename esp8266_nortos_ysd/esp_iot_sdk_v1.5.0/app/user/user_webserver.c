...
extern void user_esp_platform_set_connect_status(int n);
extern int atoi(const char *nptr);
extern void jsontree_write_string(const struct jsontree_context *js_ctx, const char *text);
extern int user_esp_platform_get_connect_status();
extern void user_esp_platform_set_token(char * buffer);
extern void user_esp_platform_check_ip();
/******************************************************************************
...
wifi_station_get(struct jsontree_context *js_ctx)
{
    ...
    if (os_strncmp(path, "ssid", 4) == 0) {
        jsontree_write_string(js_ctx, (char *)sta_conf->ssid);
    } else if (os_strncmp(path, "password", 8) == 0) {
        jsontree_write_string(js_ctx, (char *)sta_conf->password);
    } else if (os_strncmp(path, "ip", 2) == 0) {
        os_sprintf((char *)buf, IPSTR, IP2STR(&ipconfig.ip));
        jsontree_write_string(js_ctx, (char *)buf);
    } else if (os_strncmp(path, "mask", 4) == 0) {
        os_sprintf((char *)buf, IPSTR, IP2STR(&ipconfig.netmask));
        jsontree_write_string(js_ctx, (char *)buf);
    } else if (os_strncmp(path, "gw", 2) == 0) {
        os_sprintf((char *)buf, IPSTR, IP2STR(&ipconfig.gw));
        jsontree_write_string(js_ctx, (char *)buf);
    }
...
wifi_softap_get(struct jsontree_context *js_ctx)
{
    ...
    if (os_strncmp(path, "ssid", 4) == 0) {
        jsontree_write_string(js_ctx, (char *)ap_conf->ssid);
    } else if (os_strncmp(path, "password", 8) == 0) {
        jsontree_write_string(js_ctx, (char *)ap_conf->password);
    } else if (os_strncmp(path, "channel", 7) == 0) {
        jsontree_write_int(js_ctx, (int)ap_conf->channel);
...
scan_get(struct jsontree_context *js_ctx)
{
    ...
    if (os_strncmp(path, "TotalPage", 9) == 0) {
        ...
        os_sprintf((char *)buffer, MACSTR, MAC2STR(bss->bssid));
        jsontree_write_string(js_ctx, (char *)buffer);
        //}
    } else if (os_strncmp(path, "ssid", 4) == 0) {
        //if (bss != NULL)
        jsontree_write_string(js_ctx, (char *)bss->ssid);
...
Content-Length: 0\r\nServer: lwIP/1.4.0\r\n\n");
    ...
    if (psend) {
        ...
        espconn_secure_sent(ptrespconn, (uint8 *)pbuf, length);
#else
        espconn_sent(ptrespconn, (uint8 *)pbuf, length);
#endif
    } else {
#ifdef SERVER_SSL_ENABLE
        espconn_secure_sent(ptrespconn, (uint8 *)httphead, length);
#else
...
local_upgrade_download(void * arg,char *pusrdata, unsigned short length)
{
    ...
    if (totallength == 0 && (ptr = (char *)os_strstr(pusrdata, "\r\n\r\n")) != NULL &&
        ...
        system_upgrade((uint8 *)ptr + 4, (uint16)length);
        ...
        totallength += length;
        system_upgrade((uint8 *)pusrdata, (uint16)length);
    }
            ...
            case GET:
                ...
                if (os_strcmp(pURL_Frame->pSelect, "client") == 0 &&
                    ...
                    if (os_strcmp(pURL_Frame->pFilename, "status") == 0) {
                        ...
                        char *strstr1 = NULL;
                        strstr1 = (char *)os_strstr(pusrdata, "&");
                        ...
                        if (strstr1 == NULL) {
                            ...
                            if (os_strncmp(strstr1, "page", 4) == 0) {
                                if (pscaninfo != NULL) {
                                    pscaninfo->pagenum = *(strstr1 + 5);
