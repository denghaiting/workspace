...
def combine_bin(file_name,dest_file_name,start_offset_addr,need_chk):
    ...
    if file_name:
		...
		if need_chk:
                ...
                print '%s:tmp_len = %d(0x%x), data_len = %d(0x%x)'%(file_name,tmp_len,tmp_len,data_len,data_len)
                write_file(dest_file_name,data_bin)
                for loop in range(len(data_bin)):
                    chk_sum_new ^= ord(data_bin[loop])
                #print '----------------------------------------------------chk_new: 0x%x'%(chk_sum_new)
		...
		if need_chk:
		    for loop in range(len(data_bin)):
                ...
                if tmp_len:
                    ...
                    for loop in range(len(data_bin)):
                        chk_sum_new ^= ord(data_bin[loop])
                    #print '--------%s----------------00 len = %d'%(file_name,tmp_len)
		    if need_chk:
			for loop in range(len(data_bin)):
			    chk_sum ^= ord(data_bin[loop])
                #print '----------------------------------------------------chk_new: 0x%x'%(chk_sum_new)
...
def gen_appbin():
    ...
    sum_size = os.path.getsize(flash_bin_name) + 2
    #print '-------------------------sum_size===========%d'%(sum_size)
    ...
    if sum_size:
        ...
        for loop in range(len(data_bin)):
            chk_sum_new ^= ord(data_bin[loop])
    write_file(flash_bin_name,chr(chk_sum_new & 0xFF))
    write_file(flash_bin_name,chr(chk_sum & 0xFF))
    #print '------0x%x------------chk_sum ============0x%x'%(chk_sum_new,chk_sum)
    fp = open(flash_bin_name,'rb')
    if fp:
        fp.seek(0,os.SEEK_END)
        data_len = fp.tell()
    print '%s total: %d(%dk)Byte'%(flash_bin_name,data_len,data_len/1024)
    if boot_mode == '1':
